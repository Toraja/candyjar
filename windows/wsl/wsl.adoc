...
[NOTE]
====
Newer versions of WSL support GUI applications natively, So `DISPLAY` environment variable is not necessary. +
Refer to link:https://docs.microsoft.com/en-us/windows/wsl/tutorials/gui-apps[this page] for more detail. +
However, to run GUI/X applications from inside docker containers, setting `DISPLAY` is still necessary. +
If you just need to sync clipboard, tmux does that for you if `set-clipboard on` is set in `.tmux.conf`.
====

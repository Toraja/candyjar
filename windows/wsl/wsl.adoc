...
WSL is now bundled with WSLg and supports GUI applications natively so X server apps (such as `XLaunch`) is no longer necessary. +
Also `DISPLAY` environment variable is set automatically. +
Refer to link:https://docs.microsoft.com/en-us/windows/wsl/tutorials/gui-apps[this page] for more detail. +

To run GUI applications inside containers, set `DISPLAY` variable to the same value in the container and mount some files. +
[source,shell]
----
docker run --interactive --tty --rm --env DISPLAY --volume /tmp/.X11-unix:/tmp/.X11-unix --volume /mnt/wslg:/mnt/wslg <image>
----
...
Refer to link:https://learn.microsoft.com/en-us/windows/wsl/wsl-config[this page] for more detail.
